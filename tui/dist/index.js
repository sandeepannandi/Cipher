...
var require_InputBox = __commonJS({
  "src/components/InputBox.jsx"(exports2, module2) {
    ...
    var COMMANDS = [
      ...
      { cmd: "pentest --browser --url http://localhost:8080", desc: "Browser mode: headless Chrome renders + drives JS-heavy/SPA pages + form fuzz pass proving stored/reflected XSS (M9.3.2)" },
      { cmd: "pentest --browser --url http://localhost:8080 --config app.yaml", desc: "Browser mode + config: authentication.browser login in the real JS UI" },
      { cmd: "pentest --openapi spec.yaml --url http://localhost:8080", desc: "Schema-aware (M9.3.1): OpenAPI 3/Swagger 2 \u2192 schema-driven sweep targets + mass-assignment field lists" },
...
var require_CommandHelp = __commonJS({
  "src/components/CommandHelp.jsx"(exports2, module2) {
    ...
    var COMMANDS = [
      ...
      { cmd: "/pentest --max-tokens 2000000", desc: "Pentest with an LLM token budget (default 2M) \u2014 sub-agents force a wrap-up summary instead of hard-failing when it runs out (M9.1)" },
      { cmd: "/pentest --max-cost 5.0", desc: "Pentest with a USD cost gate \u2014 refuses to start if the estimated run cost exceeds it (M9.1)" },
      { cmd: "/pentest --url http://localhost:8080", desc: "Live mode: HTTP tools + exploit validators (no exploit, no report) \u2014 M9.4 attack chaining links unlock proofs (mass-assignment user creation, JWT impersonation) to auth-blocked re-sweeps and reports \u201CAttack Paths\u201D" },
      ...
      { cmd: "/pentest --url http://localhost:8080 -w myapp", desc: "Live pentest in a named workspace: checkpoints + per-mission transcripts" },
      { cmd: "/pentest --url http://localhost:8080 --resume myapp", desc: "Resume an interrupted workspace run (skips completed missions)" },
      { cmd: "/pentest --url http://localhost:8080 --format md", desc: "Live pentest \u2192 Security-Assessment-Report.md (exec summary + PoC steps)" },
      { cmd: "/pentest --url http://localhost:8080 --format sarif", desc: "Live pentest \u2192 SARIF 2.1.0 (rule-per-bug-class, GitHub code scanning)" },
      ...
      { cmd: "/pentest --config app.yaml", desc: "M9.1 adaptive config: models (default/cheap/escalate_after_turns), cost (per-1M pricing), budget (max_tokens/max_cost/condense_threshold) \u2014 cheap model first, strong model for the deep window, cost gate before the first token" },
      ...
      { cmd: "/pentest --browser --url http://localhost:8080", desc: "Browser mode: headless Chrome renders + DRIVES JS-heavy/SPA pages \u2014 render_page, browser_action (fill/click/submit), cookie bridge, DOM-XSS/clickjacking validators + a deterministic form fuzz pass proving stored/reflected XSS on marker execution (M9.3.2) (install Chrome or set CIPHER_AI_CHROME)" },
      { cmd: "/pentest --browser --url http://localhost:8080 --config app.yaml", desc: "Browser mode + config: authentication.browser: true logs in through the real JS UI (selectors) and imports the session cookie into the shared jar" },
      { cmd: "/pentest --openapi spec.yaml --url http://localhost:8080", desc: "Schema-aware (M9.3.1): import OpenAPI 3 / Swagger 2 \u2192 schema-derived sweep targets + mass-assignment field lists; live runs auto-discover openapi.json / introspect /graphql" },
      ...
      { cmd: "/fix --list", desc: "List fixable findings \u2014 proven pentest findings from workspaces now appear" },
      { cmd: "/zeroday", desc: "3-layer zero-day anomaly detection" },
      { cmd: "/zeroday --ai", desc: "Zero-day + AI-powered analysis" },
      { cmd: "/zeroday --anomaly-only", desc: "Zero-day: anomaly layer only" },
      { cmd: "/zeroday --no-flow", desc: "Zero-day: skip taint flow analysis" },
...
var require_CommandPalette = __commonJS({
  "src/components/CommandPalette.jsx"(exports2, module2) {
    ...
    var COMMANDS = [
      ...
      { cmd: "pentest --browser --url http://localhost:8080", desc: "Browser mode: headless Chrome renders + drives JS-heavy/SPA pages + a deterministic form fuzz pass proving stored/reflected XSS on marker execution (M9.3.2)" },
      { cmd: "pentest --browser --url http://localhost:8080 --config app.yaml", desc: "Browser mode + config: authentication.browser login in the real JS UI, session cookie imported into the shared jar" },
      { cmd: "pentest --openapi spec.yaml --url http://localhost:8080", desc: "Schema-aware (M9.3.1): import OpenAPI 3/Swagger 2 \u2192 schema-derived sweep targets + mass-assignment field lists" },
      { cmd: "watch --pentest http://localhost:8080", desc: "Watch + live exploit sweep against a URL each scan (no LLM)" },
