...
| `cipher-ai fix --list` | Pentest findings from workspaces now appear — code-anchored ones patch via AI-fix → verify → `--pr` |  | `cipher-ai pentest --blackbox --url http://localhost:8080` | **Black-box mode** — crawl the live target (bounded BFS: same-origin, page/depth caps, form discovery) and sweep every discovery — no source, no AI key |
  | `cipher-ai pentest --openapi spec.yaml --url http://localhost:8080` | **Schema-aware (M9.3.1)** — import an OpenAPI 3 / Swagger 2 spec (JSON or YAML): schema-derived sweep targets with type-aware params + mass-assignment field lists from request-body properties. Without a file, live runs auto-discover `openapi.json`/`swagger.json` and introspect `/graphql` |
  | `cipher-ai pentest --browser --url http://localhost:8080` | **Browser mode (M8.6 + M8.7 + M9.3.2)** — headless Chrome/Chromium/Edge renders JS-heavy & SPA pages (`render_page` tool, crawler discovers client-side forms/links) AND **drives** them: the agent fills forms, clicks, submits, and captures cookies with `browser_action`; browser cookies bridge into the shared session so raw-HTTP tools stay authenticated; DOM XSS and clickjacking are proven in a real engine, and the deterministic **browser fuzz pass** submits every discovered form (incl. JS-only ones) and proves stored/reflected XSS on real marker execution (install Chrome or set `CIPHER_AI_CHROME`; allowlist + scope still gate every action) |
...
**Phase 11 / M8.6** closes the JS-heavy / SPA gap without Playwright or Node: **`--browser`** drives your locally-installed headless Chrome/Chromium/Edge (Windows, WSL interop, macOS, Linux — override with `CIPHER_AI_CHROME`) to render pages and feed the rendered DOM into the same parsers. The agent gains a **`render_page` tool**, and the black-box crawler's `browser` mode discovers forms/links that only exist after client-side JS runs (falling back to raw HTML when no browser is installed). Safety is unchanged: the `--allow-host` allowlist is checked before Chrome even spawns (it bypasses the session client), the scope gate applies, and renders are hard-killed on a 20s timeout.

**Phase 15.3 / M9.3.1** stops guessing endpoints: the sweep imports the API's own contract. `--openapi spec.yaml|json` parses **OpenAPI 3 or Swagger 2** (JSON or YAML, no new deps) with in-document `$ref` resolution — every path+operation becomes a sweep target with type-appropriate sample params, and the request body's property list feeds the **mass-assignment** validator (schema fields probed beyond the built-in privileged list, persisted through `ProofSpec` for point re-test). Live `--url` runs **auto-discover** `openapi.json` / `swagger.json` / `/api-docs` / `v3/api-docs` at the target root and **introspect `/graphql`** into operation skeletons (`query { search(q: "1") }`) the validators inject into. Schema targets are swept first (a spec is the app's own contract), the sweep cap grows to cover them, `--blackbox` merges the contract with the crawl, and `--plan-only` lists a local spec with zero requests. All discovery probes go through the shared session (allowlist + rate limit enforced) and fail silently — a missing contract never breaks a run.

**Phase 16 / M9.4** makes the pentester *think in paths*: **multi-step attack chaining**. Every proven proof is mapped to the capability it unlocks (a **primitive** — mass-assignment user creation, JWT impersonation, SSRF/file-read/command-exec). When a first-pass proof unlocks a primitive, the endpoints the sweep found **auth-blocked (401/403)** are re-probed **once** with the now-primed session: the dependent finding (e.g. **user-creation → IDOR**) fires only if the unlock actually flipped the auth gate — a deterministic chain with no AI key, bounded to 2 hops. Each detected chain also spawns a focused **continuation agent** (seeded with the achieved state + the chained endpoint, budgeted like any mission), and the Markdown report gains an **Attack Paths** section (unlock step → dependent step, endpoints + unlocked capability) while SARIF stays per-finding. Chains persist through the workspace checkpoint, so `--resume` and complete-session re-renders keep them.

**Phase 15.4 / M9.3.2** closes the deeper proof engine: the **browser fuzz pass**. With `--browser`, the deterministic sweep no longer stops at response bodies — it *submits* every discovered form in headless Chrome (including the **JS-only forms** the rendered-DOM crawler finds) with marker payloads and proves stored/reflected XSS **only when the marker executes on `window`** in the real engine — a sink raw HTTP can never observe. Bounded like the guided sweep, allowlist + scope gated before every navigation, seeded from both the guided pass's discovered pages and the black-box crawl, and reported as normal proven findings (correlation/checkpoints/`--point-retest` included).

**Phase 12 / M8.7** — the last Shannon-parity gap — upgrades `--browser` from *rendering* to *driving* with **zero new dependencies**: a new `src/pentest/cdp.rs` speaks the **Chrome DevTools Protocol directly over WebSocket** (hand-rolled RFC 6455 frames; Chrome launches with `--remote-debugging-port=0`, the ephemeral port read from `DevToolsActivePort`). The agent gains a **`browser_action` tool** (`navigate`, `fill` — native setter, React-friendly — `click`/`click_text`, `submit`, `wait`, `eval`, `screenshot`, `cookies`) that works the live DOM like a human pentester. A **cookie bridge** imports `Network.getAllCookies` into the shared `HttpSession` jar after every action, so `http_request`/`exploit`/the guided sweep stay authenticated in lockstep — *one session, two transports*. Config `authentication.browser: true` drives JS-rendered logins (and email-OTP / magic-link flows) in the real UI with `success_condition` verification. The `exploit` tool adds **in-browser proof validators**: DOM XSS (payload executes in the real JS engine, `screenshot` attached to the proof — a sink raw HTTP can never observe) and clickjacking (frame-embed test). Safety is unchanged: the allowlist + scope gate are checked before the browser does anything, and the child process is killed on drop/timeout. Note: the original pipe transport (`--remote-debugging-pipe`) is unusable when a Windows Chrome runs under WSL interop — the port transport works everywhere, and that broken combo is auto-detected with a clear hint.
