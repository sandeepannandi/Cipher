...
| `cipher-ai fix --list` | Pentest findings from workspaces now appear — code-anchored ones patch via AI-fix → verify → `--pr` |  | `cipher-ai pentest --blackbox --url http://localhost:8080` | **Black-box mode** — crawl the live target (bounded BFS: same-origin, page/depth caps, form discovery) and sweep every discovery — no source, no AI key |
  | `cipher-ai pentest --browser --url http://localhost:8080` | **Browser mode** — headless Chrome/Chromium/Edge renders JS-heavy & SPA pages: the agent gets a `render_page` tool and the black-box crawler discovers client-side forms/links the raw HTML hides (install Chrome or set `CIPHER_AI_CHROME`; allowlist + scope still gate every render) |
