...
- **Pentest integration coverage** — new `tests/pentest.rs`: a scripted agent runs the `exploit` tool through the real `ToolRegistry`/`HttpSession`/`AgentLoop` stack against a live vulnerable fixture (SQLi + XSS + weak-cookie endpoints), asserting proven findings end-to-end and an honest negative on clean endpoints
