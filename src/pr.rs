...
pub(crate) fn render_pr_comment(
    ...
    md.push_str(&format!(
        "## 🔒 CipherAI Security Review — PR #{pr_number}\n\n"
    ));
    ...
    md.push_str(&format!(
        "_Generated by [CipherAI](https://github.com/sandeepannandi/Cipher) — {attack_count} taint/attack chains detected._\n\n"
    ));
    ...
    md.push_str("\n---\n");
    md.push_str(&format!(
        "_[View full report](https://github.com/{repo}/actions) · Run `cipher-ai review --format sarif` for machine-readable output._\n"
    ));
...
pub(crate) async fn post_pr_comment(
    ...
    if !response.status().is_success() {
        ...
        anyhow::bail!("GitHub API error ({status}) posting to {repo}#{pr_number}: {text}");
    }
...
pub(crate) async fn create_pull_request(
    ...
    if !response.status().is_success() {
        let status = response.status();
        let text = response.text().await.unwrap_or_default();
        anyhow::bail!("GitHub API error ({status}) creating PR on {repo}: {text}");
    }
...
pub(crate) async fn get_pr_changed_files(
    ...
    loop {
        let url =
            format!("{GITHUB_API}/repos/{repo}/pulls/{pr_number}/files?per_page=100&page={page}");
        let response = client
            .get(&url)
            .bearer_auth(token)
            .header("Accept", "application/vnd.github+json")
        ...
        if !response.status().is_success() {
            ...
            let text = response.text().await.unwrap_or_default();
            anyhow::bail!(
                "GitHub API error ({status}) fetching files for {repo}#{pr_number}: {text}"
            );
        }
...
pub(crate) async fn post_review_comments(
    ...
    if !response.status().is_success() {
        let status = response.status();
        let text = response.text().await.unwrap_or_default();
        anyhow::bail!("GitHub API error ({status}) posting review to {repo}#{pr_number}: {text}");
    }
    Ok(())
}
...
pub async fn run_pr(
    ...
    if diff_only {
        ...
        match get_pr_changed_files(&repo, &token, pr_number).await {
            ...
            Err(e) => {
                output::print_warn(
                    "Diff",
                    &format!("could not fetch changed files ({e}); reviewing full repository"),
                );
            }
        }
    }
    ...
    match post_pr_comment(&repo, pr_number, &token, &comment).await {
        Ok(()) => {
            output::print_success(&format!("Comment posted to {repo}/pull/{pr_number}"));
        }
        Err(e) => {
