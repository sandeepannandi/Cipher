...
pub async fn run_review(
    ...
    println!();
    println!(
        "  [IDEA] Run {} for interactive security Q&A ]);
        assert!(found.is_empty(), "{found:?}");
    }
...
resp = requests.get(endpoint, timeout=5)"#,
        ...
        as  if let Some(captures) = binding.captures(code) {
            let lhs = captures.get(1).map(|capture| capture.as_str());
            let rhs = captures
                .get(2)
                .map(|capture| capture.as_str())
...
fn flow_functions(lines: &[&str], language: FlowLanguage) -> Vec<FlowFunction> {
    ...
    let headers: Vec<(Regex, bool)> = match language {
        ...
        FlowLanguage::Java => vec![(
            r#"^\s*(?:(?:public|private|protected|static|final|synchronized|abstract)\s+)*(?:<[^>]+>\s+)?(?:[A-Za-z_][A-Za-z0-9_.<>\[\], ?]*?\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*\(([^()]*)\)\s*(?:throws\s+[A-Za-z0-9_.,\s]+?)sert_eq!(titles(&findings), vec![SSRF]);
        assert_eq!(findings[0].line_number, Some(3));
    }
    ...
    match language {
        FlowLanguage::JavaScript => {
            ...
            let (
                ...
                Regex::new(
                    r#"^\s*import\s+([A-Za-  if let Some(captures) = binding.captures(code) {
            let lhs = captures.get(1).map(|capture| capture.as_str());
            let rhs = captures
                .get(2)
                .map(|capture| capture.as_str())
...
fn flow_functions(lines: &[&str], language: FlowLanguage) -> Vec<FlowFunction> {
    ...
    let headers: Vec<(Regex, bool)> = match language {
        ...
        FlowLanguage::Java => vec![(
            r#"^\s*(?:(?:public|private|protected|static|final|synchronized|abstract)\s+)*(?:<[^>]+>\s+)?(?:[A-Za-z_][A-Za-z0-9_.<>\[\], ?]*?\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*\(([^()]*)\)\s*(?:throws\s+[A-Za-z0-9_.,\s]+?)z_$][A-Za-z0-9_$]*)\s*,\s*\{([^}]*)\}\s*from\s+['"]([^'"]+)['"]\s*;?\s*$"#,
                ),
            )
            else {
                return bindings;
