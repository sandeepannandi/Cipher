...
///fault_import_without_default_export_is_not_resolved() {
...
fn flow_pass(
    ...
    let (Some(source), Some(binding)) = (flow_source_regex(language), flow_binding_regex(language))
        ...
        return (sink_l package entry resolved through `node_modules`). Pattern rules stay off:
    ...
    if format == "sarif" || format == "json" {
        ...
        if let Some(out_path) = output {
            ...
            println!(
                ...
                out_path.yeines, callee_sink_lines, imported_sink_lines);
    };
